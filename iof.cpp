...
bool initialization (string test, double& dens_L, double& vel_L, double& pres_L, \
                                  double& dens_R, double& vel_R, double& pres_R, double& time)
{
    ifstream input(test);
    if (!input.is_open())
    {
    ...
    if (!(input >> dens_L >> vel_L >> pres_L >> dens_R >> vel_R >> pres_R >> time))
    {
...
bool save_results (string test, int N, double* x, double* pressure, double* density, double* velocity)
{
    string result = "";
    if (test == "input1.txt")
    {
    ...
    if (output.is_open())
    {
        for (size_t i = 0; i != N; ++i)
            output << x[i] << " " << pressure[i] << " " << density[i] << " " << velocity[i] << '\n';
    }
    else
    {
